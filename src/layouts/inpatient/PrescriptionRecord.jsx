...
const PrescriptionDialog = ({
  ...
  const handleAddToCart = async (prescriptionId, ehrId) => {
    try {
      const response = await prescriptionRecordApi.checkOutPrescriptionRecord(
        prescriptionId,
        ehrId
      );
  ...
  const handleCreatePrescription = async () => {
    try {
      ...
      const response = await prescriptionRecordApi.createNewPrescriptionRecord(
        updatedRecord,
        electronicHealthRecordId,
      ...
      reduxDispatch(
        displayMessage({
          ...
          title: "Error",
          content:
            "Failed to create a new prescription record. Please try again.",
        })
      );
    }
  };
  ...
  const fetchPrescriptionRecords = async () => {
    try {
      const response = await prescriptionRecordApi.getAllPrescriptionRecord(
        electronicHealthRecordId
      );
  ...
  const handleDelete = async (prescriptionRecordId) => {
    ...
    if (confirmDelete) {
      try {
        await prescriptionRecordApi.deletePrescriptionRecord(
  ...
  return (
    <Dialog open={open} onClose={onClose} fullWidth maxWidth="md">
      <DialogTitle>
        ...
        <IconButton onClick={fetchPrescriptionRecords} aria-label="refresh">
          <RefreshIcon />
        </IconButton>
      </DialogTitle>
                ...
                >
                  ...
                  <CardContent>
                    ...
                    <div style={{ marginTop: 10 }}>
                      ...
                      <div>
                        <b>Expiration Date:</b>{" "}
                        {formatDate(prescriptionRecord.expirationDate)}
                      </div>
                      <div>
                        <b>Medication Name:</b>{" "}
                        {prescriptionRecord.medicationName}
                      </div>
                      {/* <div>
                      ...
                      <div>
                        <b>Prescribed By:</b> {prescriptionRecord.prescribedBy}
                      </div>
                      <div>
                        <b>Last Collect Date:</b>{" "}
                        {prescriptionRecord.lastCollectDate
                          ? dayjs(
                              prescriptionRecord.lastCollectDate,
                              "YYYY-MM-DD HH:mm:ss"
                            ).format("DD/MM/YYYY")
                      ...
                      <div>
                        ...
                        {renderStatusWithColor(
                          prescriptionRecord.prescriptionStatusEnum
                        )}
                      </div>
                    </div>
                  </CardContent>
      ...
      <Dialog open={openForm} onClose={handleCloseForm} fullWidth maxWidth="md">
        ...
        <DialogContent dividers>
          ...
          <TextField
            ...
            onChange={(e) =>
              handleCreateFieldChange("medicationQuantity", e.target.value)
            }
            fullWidth
            margin="normal"
          />
          ...
          <TextField
            ...
            onChange={(e) =>
              handleCreateFieldChange("comments", e.target.value)
            }
            fullWidth
            margin="normal"
          />
          ...
          <Select
            ...
            onChange={(e) =>
              handleCreateFieldChange("prescriptionStatusEnum", e.target.value)
            }
            displayEmpty
            fullWidth
            margin="normal"
