...
function FacilityBooking() {
  ...
  const [bookingToDeleteStartDate, setBookingToDeleteStartDate] =
    useState(null);
  const [
    isDeleteConfirmationCalendarOpen,
    setIsDeleteConfirmationCalendarOpen,
  ] = useState(false);
    ...
    facilityApi
      ...
      .then((response) => {
        ...
        const mappedRows = facilities.map((facility) => ({
          ...
          startDateTime: moment(new Date(facility.startDateTime)).format(
            "MMMM Do YYYY, h:mm:ss a"
          ),
          endDateTime: moment(new Date(facility.endDateTime)).format(
            "MMMM Do YYYY, h:mm:ss a"
          ),
  ...
  const [bookingData, setBookingData] = useState({
    columns: [
      {
        Header: "Facility Booking ID",
        accessor: "facilityBookingId",
        width: "10%",
      },
        ...
        Cell: ({ row }) => (
          <MDBox>
            <IconButton
              color="secondary"
              onClick={() =>
                handleDeleteFacilityBooking(
                  row.original.facilityBookingId,
      ...
      facilityApi
        ...
        .then((response) => {
          const updatedEvents = calendarEvents.filter(
            (event) => event.facilityBookingId !== id
          );
          fetchData();
          fetchBookingData();
  ...
  const handleDeleteFacilityBookingCalendar = (
    facilityBookingId,
    startDateTime
  ) => {
  ...
  const handleConfirmDeleteBooking = (facilityBookingId, startDateTime) => {
    try {
      ...
      console.log(currentDate);
      // console.log(bookingDate);
      const dateComponents = startDateTime.match(
        /(\w+) (\d+)(?:st|nd|rd|th)? (\d+), (\d+):(\d+):(\d+) ([APap][Mm])/
      );
        ...
        .then(() => {
          ...
          reduxDispatch(
            displayMessage({
              ...
              content:
                "Facility Booking with facilitybooking Id: " +
                facilityBookingId +
                " deleted",
            })
          );
        ...
        if (event.event.staffUsername === staff.username) {
          ...
          reduxDispatch(
            displayMessage({
              ...
              icon: "notification",
              title:
                "Permission denied: You cannot resize an event you didn't create.",
            })
          );
        }
      }
        ...
        if (event.event.staffUsername === staff.username) {
          ...
          reduxDispatch(
            displayMessage({
              ...
              icon: "notification",
              title:
                "Permission denied: You cannot move a booking you didn't create.",
            })
          );
        }
      }
          ...
          <Grid item xs={12}>
            <Card>
              ...
              <MDBox pt={3}>
                ...
                >
                  ...
                  <DialogContent>
                    <DnDCalendar
                      ...
                      {...calendarConfig}
                      onSelectEvent={(event) =>
                        handleEventClick(event, event.facilityBookingId)
                      }
                      selectable
                      onSelectSlot={handleSlotSelect}
                      eventPropGetter={eventStyleGetter}
                      onEventResize={(event) =>
                        handleEventResize(event, event.facilityBookingId)
                      }
                    />
                  </DialogContent>
                </Dialog>
                      ...
                      <>
                        ...
                        <ListItem>
                          ...
                          <MDTypography
                            ...
                            style={{ padding: "8px" }}
                          >
                            {moment(selectedBooking.end).format(
                              "MMMM Do YYYY, h:mm:ss a"
                            )}
                          </MDTypography>
                        </ListItem>
                        ...
                        {selectedBooking.staffUsername === staff.username && (
                          <Button
                            ...
                            onClick={() =>
                              handleDeleteFacilityBookingCalendar(
                                selectedBooking.facilityBookingId,
                ...
                <Dialog
                  open={isDeleteConfirmationCalendarOpen}
                  onClose={() => setIsDeleteConfirmationCalendarOpen(false)}
                >
                  <DialogTitle>Confirm Deletion</DialogTitle>
                  <DialogContent>
                    Are you sure you want to delete this booking?
                  ...
                  <DialogActions>
                    <Button
                      onClick={() => setIsDeleteConfirmationCalendarOpen(false)}
                      color="primary"
                    >
                      Cancel
                    </Button>
