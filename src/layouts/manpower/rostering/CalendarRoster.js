...
function CalendarRoster() {
  ...
  const getOverallRoster = async (id) => {
    try {
      const response = await shiftApi.viewOverallRoster(
      ...
      for (let i = 0; i < response.data.length; i++) {
        ...
        data.push({
          ...
          title: getShiftName(
            getTime(response.data[i].startTime),
            getTime(response.data[i].endTime)
          ),
      ...
      for (let i = 0; i < responseLeaves.data.length; i++) {
        if (responseLeaves.data[i].approvalStatusEnum !== "REJECTED") {
          data.push({
            ...
            start: new Date(
              responseLeaves.data[i].startDate[0],
              responseLeaves.data[i].startDate[1] - 1,
              responseLeaves.data[i].startDate[2],
              0,
              0,
              0,
              0
              ...
              responseLeaves.data[i].endDate[0],
              responseLeaves.data[i].endDate[1] - 1,
              responseLeaves.data[i].endDate[2],
              23,
              59,
              0,
              0
  ...
  const getShiftPreference = async () => {
    try {
      const response = await shiftPreferenceApi.getShiftPreference(
        localStorage.getItem("staffUsername")
      );
      ...
      if (response.data) {
        setShift(
          getShiftId(
            moment(response.data.startTime, "HH:mm:ss").format("HH:mm"),
            moment(response.data.endTime, "HH:mm:ss").format("HH:mm")
          )
        );
      } else {
        setShift(0);
  ...
  const handleSubmit = async (event) => {
    ...
    if (shift == 0) {
      ...
      try {
        const response = await shiftPreferenceApi.createShiftPreference(
          newReqBody
        );
        reduxDispatch(
          displayMessage({
            ...
            content:
              "Shift preference with start time " +
              newReqBody.startTime +
              " and end time " +
              newReqBody.endTime +
              " has been added!",
          })
  ...
  const getStaffByUsername = async () => {
    try {
      const response = await staffApi.getStaffByUsername(
        localStorage.getItem("staffUsername")
      );
              ...
              <AccordionDetails>
                <FormControl>
                  <Typography variant="h6">
                    ...
                    {currPref
                      ? currPref.startTime + " - " + currPref.endTime
                      : "No preference"}{" "}
                  </Typography>
                  <RadioGroup
                  ...
                  >
                    ...
                    <FormControlLabel
                      value={1}
                      control={<Radio />}
                      label="Shift 1 (00:00 - 08:00)"
                    />
                    <FormControlLabel
                      value={2}
                      control={<Radio />}
                      label="Shift 2 (08:00 - 16:00)"
                    />
                    <FormControlLabel
                      value={3}
                      control={<Radio />}
                      label="Shift 3 (16:00 - 23:59)"
                    />
                  </RadioGroup>
                  <Button
                    variant="contained"
